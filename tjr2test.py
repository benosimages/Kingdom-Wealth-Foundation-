"""TJR v2 test — recalibrated sweep detection, tested offline BEFORE touching the live bot.

Change under test (bot.py signal(), tjr branch): sweep is tracked across the WHOLE
day with a 0.05% near-touch tolerance, instead of only the last 8 closes.
Rejection + structure-break trigger unchanged.
...
TOL = 0.0005  # near-sweep tolerance (liquidity often sits just inside the level)

def signal_v2(series, day_series, prev):
    if not prev or len(day_series) < 5:
        return 0
    hi, lo = prev
    c = series[-1]
    last3 = day_series[-4:-1]
    if max(day_series) >= hi * (1 - TOL) and c < hi and c < min(last3):
        return -1
    if min(day_series) <= lo * (1 + TOL) and c > lo and c > max(last3):
        return 1
    return 0
...
def simulate(merged, levels, min_score, min_rr, diag=None):
    ...
    for (t, inst, o, h, l, c) in merged:
        ...
        if not prev:
            continue
        bump("bars_checked")
        sig = signal_v2(series[inst], day_series[inst], prev)
        if sig == 0:
            bump("no_signal")
            continue
        bump("signal_fired")
...
def main():
    ...
    diag = {}
    lines = [f"# TJR v2 (whole-day sweep) — last {DAYS} days · {', '.join(INSTRUMENTS)}",
    ...
    if best_trades:
        with open("tjr2_trades.csv", "w", newline="") as f:
            w = csv.writer(f)
    ...
    with open("tjr2_report.md", "w") as f:
        f.write("\n".join(lines))
    print("\n".join(lines))
